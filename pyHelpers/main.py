x = """
bg-white	background-color: rgb(248 250 252);
...
ListOfColors = []
colorName = x.split("\n")
# ["BackgroundColor3"] = Color3.fromRGB(243, 244, 246)
for color in colorName:
    if(not color):
        continue
    className = color.split("\t")[0].replace("bg", "border")
    Color3 = color.split("\t")[1].replace("background-color: rgb(", "").replace(");", "").split(" ")
    ListOfColors.append(f'["{className}"] = {{["Color"] = Color3.fromRGB({Color3[0]}, {Color3[1]}, {Color3[2]}), ["RequiredInstance"] = "UIStroke"}},')


for e in ListOfColors:
    print(e)